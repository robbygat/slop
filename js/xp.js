...
/** XP card on the profile page (replaces the old nav pill). */
export function renderProfileXP() {
  const slot = document.getElementById('pf-xp');
  if (!slot) return;
  const p = getProgress();
...
export function initXP() {
  refreshXPUI();
  window.addEventListener('slop:xp', refreshXPUI);
  window.addEventListener('slop:auth', refreshXPUI);
